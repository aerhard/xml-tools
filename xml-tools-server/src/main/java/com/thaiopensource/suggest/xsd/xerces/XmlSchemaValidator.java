    ...
    */
    ...
    public boolean characterData(String data, Augmentations augs) {
        ...
        if (fCurrentType != null
            ...
            if (ctype.getContentType() == XSComplexTypeDecl.CONTENTTYPE_ELEMENT) {
    ...
    Augmentations handleStartElement(QName element, XMLAttributes attributes, Augmentations augs) {
        ...
        if (fCurrentType == null && xsiType == null) {
            ...
            // if the type is simple, we need to append
                ...
                XSComplexTypeDecl ctype = (XSComplexTypeDecl) fCurrentType;
                fAppendBuffer = (ctype.getContentType() == XSComplexTypeDecl.CONTENTTYPE_SIMPLE);
            }
        }
        ...
        if (fCurrentType.getTypeCategory() == XSTypeDefinition.COMPLEX_TYPE) {
            ...
            if (fNormalizeData) {
                ...
                if (ctype.getContentType() == XSComplexTypeDecl.CONTENTTYPE_SIMPLE) {
                    if (ctype.getSimpleType().getVariety() == XSSimpleType.VARIETY_UNION) {
                        fUnionType = true;
                    } else if (ctype.getSimpleType() instanceof XSSimpleType){
                        try {
                            fWhiteSpace = ((XSSimpleType) ctype.getSimpleType()).getWhitespace();
        ...
        if (fIDCChecking) {
            ...
            if (fCurrentElemDecl != null && fCurrentElemDecl.getIdentityConstraints().getLength() > 0) {
                fIdConstraint = true;
    ...
    SchemaGrammar findSchemaGrammar(
        ...
        if (!fUseGrammarPoolOnly && (grammar == null ||
            ...
            fXSDDescription.setContextType(contextType);
            fXSDDescription.setNamespace(namespace);
            fXSDDescription.setEnclosingElementName(enclosingElement);
            fXSDDescription.setTriggeringComponent(triggeringComponent);
            fXSDDescription.setAttributes(attributes);
            if (fLocator != null) {
            ...
            if (grammar == null || fXSDDescription.getLocationHints() != null) {
    ...
    private void setLocationHints(XSDDescription desc, String[] locations, SchemaGrammar grammar) {
        ...
        if (grammar == null) {
            fXSDDescription.setLocationHints(new String[length]);
            System.arraycopy(locations, 0, fXSDDescription.getLocationHints(), 0, length);
        }
        else {
    ...
    private void setLocationHints(XSDDescription desc, String[] locations, StringList docLocations) {
        ...
        if (counter > 0) {
            if (counter == length) {
                fXSDDescription.setLocationHints(hints);
            }
            else {
                fXSDDescription.setLocationHints(new String[counter]);
                System.arraycopy(hints, 0, fXSDDescription.getLocationHints(), 0, counter);
            }
        }
    }
    ...
    XSTypeDefinition getAndCheckXsiType(QName element, String xsiType, XMLAttributes attributes) {
        ...
        if (fCurrentType != null) {
            ...
            if (fCurrentType.getTypeCategory() == XSTypeDefinition.COMPLEX_TYPE) {
                block |= ((XSComplexTypeDecl) fCurrentType).getProhibitedSubstitutions();
            }
    ...
    void processAttributes(QName element, XMLAttributes attributes, XSAttributeGroupDecl attrGrp) {
        ...
        for (int index = 0; index < attCount; index++) {
            ...
            for (int i = 0; i < useCount; i++) {
                ...
                if (oneUse.fAttrDecl.getName() == fTempQName.localpart
                    && oneUse.fAttrDecl.getNamespace() == fTempQName.uri) {
                    currUse = oneUse;
                    break;
                }
            }
            ...
            if (currUse != null) {
                ...
                if (currDecl == null) {
                    ...
                    if (currDecl.getTypeDefinition().getTypeCategory() == XSTypeDefinition.SIMPLE_TYPE
                        && ((XSSimpleType) currDecl.getTypeDefinition()).isIDType()) {
                        if (wildcardIDName != null) {
                            reportSchemaError(
                                "cvc-complex-type.5.1",
                                new Object[] { element.rawname, currDecl.getName(), wildcardIDName });
                        } else
                            wildcardIDName = currDecl.getName();
                    }
                }
            }
    ...
    void processOneAttribute(
        ...
        // get simple type
        XSSimpleType attDV = (XSSimpleType) currDecl.getTypeDefinition();
        ...
        try {
            ...
            if (attDV.getVariety() == XSSimpleType.VARIETY_ATOMIC
                && attDV.getPrimitiveKind() == PRIMITIVE_NOTATION) {
        ...
        if (actualValue != null && currDecl.getConstraintType() == VC_FIXED) {
            if (!ValidatedInfo.isComparable(fValidatedInfo, currDecl.getValInfo()) || !actualValue.equals(currDecl.getValInfo().actualValue)) {
                reportSchemaError(
                    "cvc-attribute.4",
                    new Object[] {
                        element.rawname,
                        fTempQName.rawname,
                        attrValue,
                        currDecl.getValInfo().stringValue()});
            }
        }
        ...
        if (actualValue != null
            && currUse != null
            && currUse.fConstraintType == VC_FIXED) {
    ...
    void addDefaultAttributes(
        ...
        for (int i = 0; i < useCount; i++) {
            ...
            if (constType == XSConstants.VC_NONE) {
                ...
                defaultValue = currDecl.getValInfo();
            }
            // whether this attribute is specified
            isSpecified = attributes.getValue(currDecl.getNamespace(), currDecl.getName()) != null;
            ...
            if (currUse.fUse == SchemaSymbols.USE_REQUIRED) {
                if (!isSpecified)
                    reportSchemaError(
                        "cvc-complex-type.4",
                        new Object[] { element.rawname, currDecl.getName() });
            }
            // if the attribute is not specified, then apply the value constraint
            if (!isSpecified && constType != XSConstants.VC_NONE) {
                attName =
                    new QName(null, currDecl.getName(), currDecl.getName(), currDecl.getNamespace());
        ...
        if (fCurrentElemDecl != null
            ...
            if (fCurrentElemDecl != null
                ...
                if (fCurrentType.getTypeCategory() == XSTypeDefinition.COMPLEX_TYPE) {
                    ...
                    if (ctype.getContentType() == XSComplexTypeDecl.CONTENTTYPE_MIXED) {
                    ...
                    else if (ctype.getContentType() == XSComplexTypeDecl.CONTENTTYPE_SIMPLE) {
    ...
    Object elementLocallyValidComplexType(QName element, Object textContent) {
        ...
        if (!fNil) {
            ...
            if (ctype.getContentType() == XSComplexTypeDecl.CONTENTTYPE_EMPTY
            ...
            // 2.2 If the {content type} is a simple type definition, then the element information item has no element information item [children], and the normalized value of the element information item is valid with respect to that simple type definition as defined by String Valid (3.14.4).
                ...
                XSSimpleType dv = (XSSimpleType) ctype.getSimpleType();
                try {
                    if (!fNormalizeData || fUnionType) {
                        fValidationState.setNormalizationRequired(true);
                    }
            ...
            else if (ctype.getContentType() == XSComplexTypeDecl.CONTENTTYPE_ELEMENT) {
                if (fSawCharacters) {
            ...
            if (ctype.getContentType() == XSComplexTypeDecl.CONTENTTYPE_ELEMENT
                || ctype.getContentType() == XSComplexTypeDecl.CONTENTTYPE_MIXED) {
    ...
    public class KeyRefValueStore extends ValueStoreBase {
        ...
        public int getFieldIndex(FieldWrapper field) {
            return Arrays.asList(fFields).indexOf(field);
        }
    ...
    public class ValueStoreCache {
        ...
        public ArrayList getValueStores() {
            return fValueStores;
        }
        ...
        public void initValueStoresFor(XSElementDecl eDecl, FieldActivator activator) {
            ...
            XSNamedMap identityConstraints = eDecl.getIdentityConstraints();
            int icCount = identityConstraints.getLength();
            for (int i = 0; i < icCount; i++) {
              IdentityConstraint item = (IdentityConstraint) identityConstraints.item(i);
                switch (item.getCategory()) {
                    case (IdentityConstraint.IC_UNIQUE) :
                        // initialize value stores for unique fields
                        UniqueOrKey unique = (UniqueOrKey) item;
                    ...
                    case (IdentityConstraint.IC_KEY) :
                        ...
                        UniqueOrKey key = (UniqueOrKey) item;
                    ...
                    case (IdentityConstraint.IC_KEYREF) :
                        KeyRef keyRef = new KeyRef(item.getNamespace(), item.getName(), item.getElementName(), ((KeyRef) item).getKey());

                        Selector selector = item.getSelector();
                        Selector nSelector = new Selector((Selector.XPath) selector.getXPath(), keyRef);
                        keyRef.setSelector(nSelector);

                        int fieldCount = item.getFieldCount();
                        for (int j = 0; j < fieldCount; j++) {
                            Field field = item.getFieldAt(j);
                            FieldWrapper fieldWrapper = new FieldWrapper((Field.XPath) field.getXPath(), keyRef);
                            keyRef.addField(fieldWrapper);
                        }
    ...
    public XPathMatcherStack getMatcherStack() {
        return fMatcherStack;
    }

    public ValueStoreCache getValueStoreCache() { return fValueStoreCache; }
